...
class CityController extends Controller
{
    ...
    public function registered(Request $registeredRequest)
    {
        ...
        if($count == 0)
        {
            $student_id = City::createUser(self::userHandle($registeredRequest));
            return  $student_id ?
    ...
    public function login(Request $request)
    {
        ...
        //方式二
        $token = auth('city')->attempt($credentials);   //获取token
//        if(!$token){
...
//        return self::respondWithToken($token, '登录成功!');   //可选择返回方式
    ...
    {
        return ['city_account' => $request['city_account'], 'password' => $request['password']];
    }
